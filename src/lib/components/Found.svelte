...
{#each foundPairs as emoji (emoji)}
	<span
		class="w- relative aspect-square rounded-lg bg-zinc-200 dark:bg-zinc-500"
		in:fade={{ delay: 500 }}
		animate:flip={{ duration: 200, delay: 500 }}
		class:w-12={foundPairs.length <= 8}
		class:w-11={foundPairs.length > 8 && foundPairs.length <= 18}
